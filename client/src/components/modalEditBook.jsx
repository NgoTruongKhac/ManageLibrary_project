...
export default function ModalEditBook({ bookId, onBookUpdated }) {
  ...
  useEffect(() => {
    const fectBook = async () => {
      try {
        const res = await apiBook.getBook(bookId);
        setBook(res.data);
      } catch (error) {
        console.error(error);
      }
  ...
  useEffect(() => {
    if (book) {
      ...
      setPublishYear(book.publicationYear || "");
      setStock(book.stock || "");
      setSelectedCategory(book.category || "");

      const sub =
        categories.find((cat) => slugify(cat.title) === book.category)?.links ||
        [];
      setSubCategories(sub);

      setSelectedSubCategory(book.subCategory || "");
      setDescription(book.description || "");
      setImage(book.coverImage || null);
      setImageFile(book.coverImage || null);
    }
  }, [book]);
  ...
  const handleUpdateBook = async () => {
    ...
    try {
      const res = await apiBook.editBook(bookId, formData);
      console.log("Sách đã được cập nhật:", res.data);
      // Reset form
      if (onBookUpdated) {
        onBookUpdated();
      }

      toast.success("thay đổi sách thành công");
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <select
            ...
            value={slugToNameMap.get(selectedCategory)}
            onChange={(e) => {
          ...
          <select
            className="select select-bordered w-full"
            value={slugToNameMap.get(selectedSubCategory)}
